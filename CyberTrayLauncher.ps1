        ...
        $MenuItem.Add_Click({
            try{
                ...
                if(Test-Path $MyScriptPath){
                    Start-Process -FilePath "C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe" -ArgumentList "-NoProfile -NoLogo -ExecutionPolicy Unrestricted -File `"$MyScriptPath`"" -ErrorAction Stop
                } else {
...
$SystrayIcon = "$psscriptroot\CyberBlackIcon.ico"
$SystrayLauncher.Icon = $SystrayIcon
$SystrayLauncher.Text = "CyberAuditTool"
$SystrayLauncher.Visible = $true

$ContextMenu = New-Object System.Windows.Forms.ContextMenu

$Build = New-MenuItem -Text "Build" -MyScriptPath "$psscriptroot\CyberBuild.ps1"
$Audit = New-MenuItem -Text "Audit" -MyScriptPath "$psscriptroot\CyberAudit.ps1"
$Analyze = New-MenuItem -Text "Analyze" -MyScriptPath "$psscriptroot\CyberAnayzers.ps1"
$ExitLauncher = New-MenuItem -Text "Exit" -ExitOnly

#Add menu items to context menu
$ContextMenu.MenuItems.AddRange($Build)
$ContextMenu.MenuItems.AddRange($Audit)
$ContextMenu.MenuItems.AddRange($Analyze)
